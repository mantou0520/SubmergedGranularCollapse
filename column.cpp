...
struct UserData
{
	...
	double thetaAngle = 30;    // contact angle
	double ParRad = 0.1;
	bool DragCalc = false;     // turn on or off the drag force
	bool CapCalc = false;      // turn on or off the capillary force
};
...
void Setup (DEM::Domain & dom, void * UD) //function that is going to be called every time step
{
	...
	for (size_t i=0; i<dom.Interactons.Size(); i++)
	{
		...
		normal /= norm(normal);
		double Vl = dat.hcutoff*dat.hcutoff*dat.hcutoff/(1+dat.thetaAngle/180.0*3.14159265);
		double Gf = 1.0-(1.0/(1.0+2*Vl/(3.14159265*Reff*dist*dist)));
		Vec3_t t1,t2,x1,x2;
		...
		{
			Fn = Fn*Gf*Gf;
			Fcap = - 4.0*3.14159265*dat.GammaSurf*Reff*cos(dat.thetaAngle/180.0*3.14159265)*Gf;  // calculate the capillary force
		}
	...
	#pragma omp parallel for schedule(static) num_threads(dom.Nproc)
	// calculate the drag force on particles

	for (size_t i=0; i<dom.Particles.Size(); i++)
	{
		dom.Particles[i]->F += -6.0*3.1415926*(dat.ParRad)*dragvisc*dom.Particles[i]->v;
	}	
}
...
{   
	...
	double hcutoff;     // cutoff distance of the lubrication force
	double hrough;      // surface roughness of particles;
	double GammaSurf;   // surface tension
	double thetaAngle;  // contact angle
	bool DragCalc;      // turn on or off the drag force
	bool CapCalc;       // turn on or off the capillary force
	{
		...
		infile >> hcutoff;      infile.ignore(200,'\n');
		infile >> hrough;       infile.ignore(200,'\n');
		infile >> GammaSurf;    infile.ignore(200,'\n');
		infile >> thetaAngle;   infile.ignore(200,'\n');
		infile >> DragCalc;     infile.ignore(200,'\n');
		infile >> CapCalc;      infile.ignore(200,'\n');
    }
	...
	if (ptype=="sphere" || ptype=="Sphere")
	{
		...
		dat.hcutoff = hcutoff;
		dat.hrough = hrough;
		dat.GammaSurf = GammaSurf;  // surface tension
		dat.thetaAngle = thetaAngle;    // contact angle
		dat.ParRad = Sphere_size/2.0;
		dat.DragCalc = DragCalc;     // turn on or off the drag force
		dat.CapCalc = CapCalc;      // turn on or off the capillary force
		dom.Load("Stage_1");
		...
		//d.WriteXDMF("test");
		dom.Solve(/*tf*/Tf, dt, /*dtOut*/dtOut, &Setup, &Report, "column_sphere", 2, Nproc);
	}
	else throw new Fatal("Packing for particle type not implemented yet");
